...
export async function GET() {
    ...
    try {
      ...
      for (const dayData of history) {
        try {
          ...
          if (validationError instanceof ZodError) {
            console.error(
              `Validation failed for historical data of ${symbol} on ${dayData.date}: ${validationError.errors
                .map((e) => e.message)
                .join(", ")}`
            )
            ...
            console.error(
              `Data validation error for historical data of ${symbol} on ${dayData.date}: ${validationError.message}`
            )
          } else {
            console.error(
              `Unknown data validation error for historical data of ${symbol} on ${dayData.date}:`,
              validationError
            )
          }
      ...
      if (rowsToInsert.length > 0) {
        try {
          ...
          if (!symbolFailed) {
            // Only add to successful if no validation errors occurred for any row
  ...
  if (failed.length > 0) {
    ...
    return NextResponse.json({
      message: "Historical data feed completed, but no data was successfully inserted.",
      successful,
      failed,
    })
  }
