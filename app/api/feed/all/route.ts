  ...
  for (const symbol of SYMBOLS_TO_FEED) {
    ...
    try {
      const quotesResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/feed/quotes`
      )
      ...
      symbolResults.push(`Quotes API fetch error: ${errorMessage}`)
    }

    // 2. Feed Historical Data (via internal API call)
    try {
      const historicalResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/feed/historical`
      )
      ...
      symbolFailed = true
      const errorMessage = error instanceof Error ? error.message : String(error)
      symbolResults.push(`Historical API fetch error: ${errorMessage}`)
    }
    ...
    try {
      ...
      symbolFailed = true
      const errorMessage = error instanceof Error ? error.message : String(error)
      symbolResults.push(`Corporate Actions ETL failed: ${errorMessage}`)
    }
    ...
    try {
      ...
      symbolFailed = true
      const errorMessage = error instanceof Error ? error.message : String(error)
      symbolResults.push(`Indicators ETL failed: ${errorMessage}`)
    }
