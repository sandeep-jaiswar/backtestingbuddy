...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, error, helperText, options, placeholder, className, id, ...props }, ref) => {
    ...
    const baseClasses =
      "w-full px-3 py-2 text-ui-text-primary bg-ui-background-primary border border-ui-border-primary rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-ui-border-focus focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed appearance-none bg-no-repeat bg-right bg-[length:16px_16px] pr-10"

    const errorClasses = error ? "border-ui-text-destructive focus:ring-ui-text-destructive" : ""

    const chevronIcon = `data:image/svg+xml;base64,${btoa(
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={selectId} className="mb-1 block text-sm font-medium text-ui-text-secondary">
            {label}
          </label>
        )}
