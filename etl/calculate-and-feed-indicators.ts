...
export async function calculateAndFeedIndicators(
  ...
  try {
    ...
    if (close.length < 200) { // Example: for SMA200, need at least 200 data points
        console.warn(`Insufficient data (${close.length} points) for comprehensive indicator calculation for ${symbol}. Skipping.`);
        return;
    }
    ...
    const getIndicatorValue = (arr: (number | { MACD?: number; signal?: number; adx?: number; k?: number; d?: number; upper?: number; lower?: number; } | undefined)[], index: number, path?: string) => {
        const val = arr[index];
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (i >= offsetSMA50) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "SMA_50", value: getIndicatorValue(sma50Values, i - offsetSMA50), params: "50", computed_time: computedTime
            }));
        }
        if (i >= offsetSMA200) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "SMA_200", value: getIndicatorValue(sma200Values, i - offsetSMA200), params: "200", computed_time: computedTime
            }));
        }
        if (i >= offsetEMA20) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "EMA_20", value: getIndicatorValue(ema20Values, i - offsetEMA20), params: "20", computed_time: computedTime
            }));
        }
        if (i >= offsetRSI14) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "RSI_14", value: getIndicatorValue(rsi14Values, i - offsetRSI14), params: "14", computed_time: computedTime
            }));
        }
        if (i >= offsetMACD) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "MACD_Line", value: getIndicatorValue(macdValues, i - offsetMACD, 'MACD'), params: "12,26,9", computed_time: computedTime
            }));
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "MACD_Signal", value: getIndicatorValue(macdValues, i - offsetMACD, 'signal'), params: "12,26,9", computed_time: computedTime
            }));
        }
        if (i >= offsetATR14) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "ATR_14", value: getIndicatorValue(atr14Values, i - offsetATR14), params: "14", computed_time: computedTime
            }));
        }
        if (i >= offsetBB) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "BB_Upper", value: getIndicatorValue(bbValues, i - offsetBB, 'upper'), params: "20,2", computed_time: computedTime
            }));
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "BB_Lower", value: getIndicatorValue(bbValues, i - offsetBB, 'lower'), params: "20,2", computed_time: computedTime
            }));
        }
        if (i >= offsetADX14) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "ADX_14", value: getIndicatorValue(adx14Values, i - offsetADX14, 'adx'), params: "14", computed_time: computedTime
            }));
        }
        if (i >= offsetStoch) {
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "Stochastic_K", value: getIndicatorValue(stochValues, i - offsetStoch, 'k'), params: "14,3", computed_time: computedTime
            }));
            indicatorsToInsert.push(IndicatorSchema.parse({
                symbol_id, ts, timeframe, name: "Stochastic_D", value: getIndicatorValue(stochValues, i - offsetStoch, 'd'), params: "14,3", computed_time: computedTime
            }));
        }
